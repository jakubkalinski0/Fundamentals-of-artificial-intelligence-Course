...
categorical_pipeline = Pipeline(steps=[("imputer", SimpleImputer(strategy="constant", fill_value="Missing")), ("encoder", one_hot_encoder)])
...
def assess_regression_model(model, X_train, X_test, y_train, y_test) -> tuple[float, float]:
    ...
    y_train_exponential = np.expm1(y_train)
    y_test_exponential = np.expm1(y_test)
    y_train_pred_exponential = np.expm1(y_train_prediction)
    y_test_pred_exponential = np.expm1(y_test_prediction)

    rmse_train = root_mean_squared_error(y_train_exponential, y_train_pred_exponential)
    rmse_test = root_mean_squared_error(y_test_exponential, y_test_pred_exponential)
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# Model uzyskał błąd RMSE na zbiorze treningowym 16751,11 USD, a na zbiorze testowym 21318,61 USD.
# Różnica między błędami wynosi ok. 4,5 tys. USD (ok. 27%), co oznacza, że model nie jest mocno przeuczony — na danych testowych wciąż osiąga podobną jakość predykcji jak na treningowych.
#
# Wartości RMSE są stosunkowo niskie w porównaniu do typowych cen domów w zbiorze Ames (średnio ok. 180–200 tys. USD).
# Oznacza to, że średni błąd predykcji stanowi około 10–12% ceny domu, co jest dobrym wynikiem w kontekście problemu regresji cen nieruchomości.
...
ridge = RidgeCV(alphas=alphas_ridge, cv=None)  # domyślnie jest LOOCV jak da się None a LOOCV jest najefektywniejsze w tym przypadku
ridge.fit(X_train, y_train)

reg_ridge_alpha = np.round(ridge.alpha_, 4)
print(f"Optymalne alpha (Ridge): {reg_ridge_alpha}")

ridge_train_rmse, ridge_test_rmse = assess_regression_model(ridge, X_train, X_test, y_train, y_test)


# L1
lasso = LassoCV(n_alphas=1000, cv=5, random_state=0, max_iter=10000)
lasso.fit(X_train, y_train)

reg_lasso_alpha = np.round(lasso.alpha_, 4)
print(f"Optymalne alpha (LASSO): {reg_lasso_alpha}")
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# Po zastosowaniu regresji regularyzowanej Ridge (L2) i LASSO (L1) udało się wyeliminować overfitting. W obu przypadkach różnica między błędem treningowym i testowym jest niewielka, co świadczy o dobrej generalizacji modeli. Ridge uzyskał minimalnie lepszy wynik RMSE, natomiast LASSO ma prostszy model dzięki wyzerowaniu nieistotnych współczynników. Średni błąd predykcji ok. 18–19 tys. USD stanowi około 10% wartości domu, co jest akceptowalnym wynikiem.
#
...
print("Kształt danych przed czyszczeniem:", df.shape)

cols_to_drop = ["default", "duration", "pdays", "poutcome"]
df = df.drop(columns=cols_to_drop, errors="ignore")
print("Po usunięciu kolumn:", df.shape)

df = df[df["education"] != "illiterate"]
print("Po usunięciu illiterate:", df.shape)
...
print("Kształt X:", df.shape)
print("Kształt y:", y.shape)
print("Liczba unikalnych wartości w y:", y.nunique())
...
if len(missing_values) > 0:
    ...
    print("Brak wartości NaN w danych.")
...
if len(unknown_counts) > 0:
    ...
    plt.title("Liczba wartości 'unknown' w kolumnach kategorycznych")
    plt.ylabel("Liczba wystąpień")
    plt.xticks(rotation=45)
    plt.show()
else:
    print("Brak wartości 'unknown' w kolumnach kategorycznych.")


# %% editable=true slideshow={"slide_type": ""} tags=["ex"]
# plot class frequencies
class_counts = y.value_counts(normalize=False)
class_percent = y.value_counts(normalize=True) * 100

bars = plt.bar(class_counts.index, class_counts.values)
plt.title('Częstość klas zmiennej docelowej (y)')
plt.xlabel('Odpowiedź klienta (czy zgodził się na lokatę)')
...
# $$
#
# Jest to ilość przypadków, w których poprawnie rozpoznaliśmy klasę pozytywną, podzielona przez ilość wszystkich przypadków z klasą pozytywną.
#
...
# bez regularyzacji
log_reg_nol = LogisticRegression(
    penalty=None,
    class_weight="balanced",
    random_state=0,
    max_iter=1000
)
log_reg_nol.fit(X_train, y_train)

y_pred_nol = log_reg_nol.predict(X_test)
...
f1_train = f1_score(y_train, log_reg_nol.predict(X_train))
f1_test = nol_f1
...
# L1
log_reg_l1 = LogisticRegressionCV(
    Cs=100,
    cv=5,
    scoring="f1",
    class_weight="balanced",
...
print(f"Brak regularyzacji: precision={nol_precision*100:.2f}%, recall={nol_recall*100:.2f}%, F1={nol_f1*100:.2f}%")
print(f"L2 regularyzacja: precision={l2_precision*100:.2f}%, recall={l2_recall*100:.2f}%, F1={l2_f1*100:.2f}%")
print(f"L1 regularyzacja: precision={l1_precision*100:.2f}%, recall={l1_recall*100:.2f}%, F1={l1_f1*100:.2f}%")
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# Nasz problem to klasyfikacja, czy klient zdecyduje się na usługi banku. W takim kontekście ważniejszy jest recall, bo chcemy wykryć jak najwięcej potencjalnie zainteresowanych klientów, nawet jeśli czasem się pomylimy.
...
pipe = Pipeline([
    ...
    ('model', LogisticRegression(penalty=None, class_weight='balanced', random_state=0, max_iter=1000))
])
...
print(f"F1 train: {f1_train*100:.2f}%, F1 test: {f1_test*100:.2f}%")
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# Regularyzacja (np. L1, L2) ma sens, gdy:
# - mamy wiele cech lub cechy są ze sobą skorelowane (ryzyko przeuczenia),
# - model osiąga bardzo dobre wyniki na treningu, ale gorsze na walidacji/testowym,
# - chcemy ograniczyć wpływ mało istotnych cech (np. L1 powoduje ich wyzerowanie).
#
# Jeśli jednak:
# - dane są niewielkie i dobrze przygotowane (mało cech, brak kolinearności),
# - model nie przeucza się (podobne wyniki na train/test).
# Wtedy regularyzacja nie przyniesie dużych korzyści.
#
# Biorąc to pod uwagę można zauwayżyć że w naszym przypadku nie ma sensu używać regularyzacji gdyż mamy bardzo podobne wyniki na train i test.
